...
function Barterer(options) {
  ...
  this.router.route('/id/:id')
    ...
    .get(function(req, res) {
      searchByIDs(self, getRequestParameters(req), function(response, status) {
        res.status(status).json(response);
      });
    });
  ...
  this.router.route('/at/:place')
    ...
    .get(function(req, res) {
      searchByPlace(self, getRequestParameters(req), function(response, status) {
        res.status(status).json(response);
      });
    });
...
function searchByIDs(instance, params, callback) {
  instance.getState( { ids: params.ids }, function(state) {
    ...
    if(foundNothing) {
      callback(responseHandler.prepareResponse(responseHandler.OK, params, {}),
