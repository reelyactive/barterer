...
class SocketManager {
  ...
  constructor(options, io, barnacles, chimps) {
    ...
    if(io) {
      io.of(DEVICES_ROUTE_REGEX).on('connection', (socket) => {
...
function handleEncryptedEvent(encrypted, activeNamespaces) {
  let deviceRoute = DEVICES_ROUTE + '/' + encrypted.deviceId + '/' +
                    encrypted.deviceIdType;

  if(activeNamespaces.has(DEVICES_ROUTE)) {
    activeNamespaces.get(DEVICES_ROUTE).emit('encrypted', encrypted);
  }
  if(activeNamespaces.has(DEVICES_ROUTE + '/encrypted')) {
    activeNamespaces.get(DEVICES_ROUTE + '/encrypted').emit('encrypted',
                                                            encrypted);
  }
  if(activeNamespaces.has(deviceRoute)) {
