...
function prepareData(response, rootUrl, data) {
  if(data.devices) {
    ...
    for(var device in devices) {
      devices[device].href = rootUrl + '/whereis/transmitter/' + device;
      var radioDecodings = devices[device].radioDecodings;
      for(var cDecoding = 0; cDecoding < radioDecodings.length; cDecoding++) {
        var radioDecoder = radioDecodings[cDecoding].identifier.value;
        radioDecodings[cDecoding].href = rootUrl + '/whatat/receiver/' +
                                         radioDecoder;
      }
    }
